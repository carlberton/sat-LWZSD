...
def test_instance(cnf_filepath, cadical_dir, timeout=None):
    """
    Tests a CNF instance with cadical and returns the result, execution time and solution.
    """
    try:
        # Path to the cadical executable
        ...
        with open(log_path, "w") as log_file:
            ...
            end_time = time.time()
            exec_time = end_time - start_time  # Time elapsed in seconds
...
def process_directory(input_dir, cadical_dir, timeout):
    ...
    with open(csv_filepath, mode='w', newline='') as csvfile:
        ...
        for filename in cnf_files:
            ...
            if result == "unsat":
                print(f"Instance {filename} is UNSAT.")
            elif result == "sat":
                print(f"Instance {filename} is SAT in {exec_time} secondes.")
            elif result == "timeout":
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Testing CNF instances with cadical.")
    ...
    parser.add_argument('--cadical', type=str, required=True, help="Cadical directory.")
